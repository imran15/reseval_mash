...
@Path("HIndexSource")
@Stateless
public class HIndexResource {

    @Context
    private UriInfo context;
    @EJB
    private HIndexLogicLocal _hIndexLocal;
    ...
    @Consumes("application/json")
    @Path("/getHIndex")
    public Response getHIndex(QueryMapperWrapper queryWrapperObj) {
        ...
        if (!(queryWrapper.getKey().equals(""))) {
            ...
            if (cacheResWrapper == null) {
                ...
                ResearchersWrapper resWrapper = _hIndexLocal.getHIndex(cacheResWrapper);
                response.setCacheKey(queryWrapper.getKey());
                response.setRequestStatus(Boolean.TRUE);
