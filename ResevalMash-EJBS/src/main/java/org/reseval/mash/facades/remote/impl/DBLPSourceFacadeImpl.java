 ...
 */
@Stateless
public class DBLPSourceFacadeImpl implements DBLPSourceFacadeRemote {
    ...
    public List<Researcher> autoCompleteResearcherName(String partialName) {
        ...
        for (Object[] row : queryRes) {
            ...
            researcher.setDblpID(((BigDecimal) row[1]).longValue());
...
//    public List<Researcher> autoCompleteResearcherName(String partialName) {
//        List<Researcher> res = new ArrayList<Researcher>();
//        Query q = emDEV.createNativeQuery("SELECT RSC_ID_PK, FullName, RSC_U_ID, DBLP_AUTHOR_ID"
//                + " FROM RESEARCHERS"
//                + " where UPPER(FullName) like :partialName AND dblp_author_id is not null"
//                + " order by FULLNAME");
...
//        Researcher researcher;
//        for (Object[] row : queryRes) {
//            if (((BigDecimal) row[3]) != null){ //checking if a research has dblp ID, then taking it otherwise not
//            researcher = new Researcher();
//            researcher.setId(((BigDecimal) row[0]).longValue());
//            researcher.setName((String) row[1]);
//            researcher.setUniversityID(((BigDecimal) row[2]).longValue());
//            researcher.setDblpID(((BigDecimal) row[3]).longValue());
//            res.add(researcher);
//            }
//            
//        }
...
//    }
    ...
    public ResearchersWrapper getPublications(ResearchersWrapper resWrapper, List<QueryMapper> queryList) {
       ...
       // System.out.println("############QUERY STRING : " + whereClause);
        if (whereClause.trim().equals("where  and") || whereClause.trim().equals("where"))
            whereClause="";
         //System.out.println("############QUERY STRING after: " + whereClause);
        List<Researcher> resList = resWrapper.getResearchers();
        for (int i = 0; i < resList.size(); i++) {

            if (resList.get(i).getDblpID() != null) {
                Query q;
                if (whereClause.length() > 7) {
                    ...
                    q = emTest_SUSAN.createNativeQuery("SELECT resource_id, resource_title, resource_uri, publication_year, citation_count, venue_id, venue_name "
                                    + " FROM MV_DBLP_RESOURCE_CITATION_INFO " + whereClause + " author_group_id = :author_dblp_id");
                            q.setParameter("author_dblp_id", resList.get(i).getDblpID());
                        } else {
                            q = emTest_SUSAN.createNativeQuery("SELECT resource_id, resource_title, resource_uri, publication_year, citation_count, venue_id, venue_name "
                                    + " FROM MV_DBLP_RESOURCE_CITATION_INFO WHERE author_group_id = :author_dblp_id");
                            q.setParameter("author_dblp_id", resList.get(i).getDblpID());
                        }
                if (fetch != -1) {
    ...
    public ResearchersWrapper getPublications(ResearchersWrapper resWrapper) {
        ...
        for (int i = 0; i < resList.size(); i++) {
            ...
            if (resList.get(i).getDblpID() != null) {
                Query q = emDEV.createNativeQuery("SELECT resource_id, resource_title, resource_uri, venue_id, venue_name "
                        + " FROM RESOURCES_INFO WHERE author_group_id = :author_dblp_id");
                q.setParameter("author_dblp_id", resList.get(i).getDblpID());
                List<Object[]> queryRes = q.getResultList();
